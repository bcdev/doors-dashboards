...
METEOGRAM_TYPES = [
    ...
    {"label": "Classical 15d with climate", "value": "classical_15d_with_climate"},
    ...
    def register_callbacks(self, component_ids: List[str], dashboard_id: str):
        ...
        def update_meteogram_image(general_data, component_data, component_state_data):
            ...
            if component_state_data is not None:
                ...
                date_string = (
                    component_data.get("date", "")
                    if component_data.get("date")
                    else None
                )
                forecast_value = component_data.get(
                    "meteogram_type_value", METEOGRAM_TYPES[0]["value"]
                )
            else:
                date_string = None
        ...
        def component_store_to_drp_label(component_data):
            if component_data is None:
                return no_update
            selected_label = component_data.get(
                "meteogram_type_label", METEOGRAM_TYPES[0]["label"]
            )
        ...
        def datepicker_to_component_store(date_value, component_data):
            ...
            selected_date = {"date": date_string}
